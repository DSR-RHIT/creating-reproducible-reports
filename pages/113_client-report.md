...
- Open a new Rmd file, and save it as `05_calibr_report.Rmd` in your `reports` directory
- Insert the same code chunk for the knitr setup 
- Start the new script with this YAML header
...
- `kable()` is a `knitr` function that produces simply tables in the output document
...
- `sprintf("format", variable)` prints the value of the variable name using the format shown, in this case, a floating point number with 3 decimal places
- placing this R code inside an inline code chunk prints the value to the output
    
To learn more, type `?sprintf` in the R console.
...
- LaTeX comments are denoted by the percent sign (%) so to print the percent symbol inside a LaTeX math expression, we have to "escape" using `\%`. 
