...
Determining the calibration equation and sensor accuracy requires that we perform a regression with output mV readings as a function of input force.   

- `input_lb` is the independent variable (*x*)
- `output_mV` is the dependent variable (*y*)

The *lm()* function creates the regression results we need, e.g., 
...
The syntax `$names` in the ttributes means that we can extract a named subset of the regression results using `$` notation.  
