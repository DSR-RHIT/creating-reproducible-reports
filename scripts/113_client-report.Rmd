...
```{r 113-04, fig.cap = 'Figure 1. Load cell calibration test setup'}
...
Inline code chunks are useful for reporting values in your prose that depend on computations. Thus the inline code chunk  \``r` `input_max` \` is replaced by the number `r input_max` in the sentence. 
...
If you type `names(data_received)` in the Console, you see that the column names use underscores, e.g., `r names(data_received)[1:3]`, etc. --- good for machine readability but not so much for human clients.   
...
```{r 113-08, fig.cap = 'Figure 2. Load cell calibration curve'}
